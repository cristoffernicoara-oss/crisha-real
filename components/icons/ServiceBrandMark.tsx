...
export function ServiceBrandMark({ brand, className }: Props) {
  const common = cn("block", className);
  ...
  switch (brand) {
    ...
    case "tiktok":
      return (
        <span
          className={cn(
            "relative inline-flex shrink-0 items-center justify-center overflow-hidden rounded-[22%] bg-black",
            className
          )}
          aria-hidden
        >
          <Image
            ...
            height={256}
            sizes="48px"
            className="h-full w-full object-contain object-center [transform:translate(-4%,-4%)]"
            aria-hidden
          />
        </span>
      );
